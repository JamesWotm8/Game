...
void Input::keyUpEvent(const SDL_Event& event){
	released_keys_[event.key.keysym.sym] = true; // when pushed it is both held down and pushed and map it accordingly
	held_keys_[event.key.keysym.sym] = false;
}
void Input::keyDownEvent(const SDL_Event& event){
	pressed_keys_[event.key.keysym.sym] = true; // when released it is released and not held down and map it accordingly
